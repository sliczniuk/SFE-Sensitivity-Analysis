...
Parameters{8} = 2;
Parameters_init_time   = [uu repmat(cell2mat(Parameters),1,N_Time)'];
[S,p,Sdot]              = Sensitivity(x, xdot, u, 47+3 );
...
for ind=0:1
    ...
    %set( cb, 'YDir', 'reverse' );
    title(cb, ['$\frac{d',name_s{ind+1},'}{d\Upsilon(0)}$'], 'Interpreter', 'latex'); cb.TickLabelInterpreter = 'latex';
    ...
    %exportgraphics(figure(1),[name_p{ind+1},'_gamma.png'], "Resolution",500);
    %close all;
end
...
for ind = 4:-1:3
    figure()
    hold on
    plot(Time, Res(end - indx,:) ./ Y .* Parameters{47}, LineWidth=2);
    %yline(0, LineWidth=2)
    xlabel('Time [min]'); ylabel(['$\frac{d ',name_s{ind+1},'}{d\Upsilon(1)} \frac{\Upsilon(0)}{y} $ '])
    hold off
    set(gca,'FontSize',My_Font)
